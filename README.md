...
- General repository baseline
  ...
  - `terraform/modules/general` が保持し、root input では上書きしない

## personal account 前提での設計方針

- `github_organization_ruleset` は使いません
- 共通 baseline (`modules/general` / `modules/advanced_security`) は singleton module として適用し、repo ごとの ruleset だけ `modules/rulesets` を `for_each` で展開します
...
- shared repository baseline (`has_wiki` / `has_issues` / merge method defaults / `delete_branch_on_merge`) は `terraform/modules/general` に固定で保持します
...
- `general` baseline は fully module-owned に変更したため、既存の `terraform.tfvars` や CI の `repositories` object から `has_wiki` と `delete_branch_on_merge` を削除してください
