...
`mise run tfinit` は `terraform/work-repositories/backend.hcl` と `terraform/fork-repositories/backend.hcl` をそのまま参照するため、事前に example からコピーして値を入れておきます。
...
2. `backend.hcl.example` を `backend.hcl` にコピーし、GCS bucket と prefix を設定します
...
backend は `terraform/work-repositories/backend.tf` で `backend "gcs" {}` の空ブロックだけを宣言し、bucket / prefix などの値は `terraform init -backend-config=...` で外から注入します。`mise run tfinit` は `terraform/work-repositories/backend.hcl` と `terraform/fork-repositories/backend.hcl` を使って両方の root module を初期化します。
...
`terraform/work-repositories/backend.hcl` と `terraform/fork-repositories/backend.hcl` はそれぞれ次のように設定します。
...
- `terraform/modules/release_please` は 1Password の `dev/release-please-bot` と `dev/metrics-token` を参照します
