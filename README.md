...
│   ├── locals.tf                  # リポジトリ定義のデフォルト値と import 対象の導出
...
- `manage_security_and_analysis = true` のときは、secure defaults として Dependabot alerts / secret scanning / push protection を有効化します
- `delete_branch_on_merge` と `has_wiki` は governance とは分けて repo ごとに保持できます
...
- `manage_security_and_analysis`: `vulnerability_alerts` と `security_and_analysis` を Terraform 管理に含めるかどうか
- `delete_branch_on_merge`: merge 後にブランチを削除するかどうか
- `has_wiki`: repo ごとの wiki 有効/無効
- `require_last_push_approval`: repo ごとの ruleset 例外として last push approval を要求するかどうか
- `main_default_ruleset_id`: 既存 `main-default` ruleset を import したい場合の ruleset ID
- `required_code_scanning`: 既存 ruleset に code scanning requirement がある repo だけ設定する optional block
